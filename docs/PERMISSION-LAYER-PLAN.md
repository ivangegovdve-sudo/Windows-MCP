...
| | `scroll`, `wait`, `waitFor` | Reversible action | Generally safe, though `scroll` can change view state. |
| | `move` | Reversible/Consequential | Reversible if `drag=False`. Consequential if `drag=True` (drag-and-drop can move/overwrite files). |
| | `shortcut` | Context-dependent (Consequential) | E.g., `Ctrl+S` vs `Alt+F4`. |
| **App** | `launch`, `switch` | Consequential action | Can interrupt user workflow. |
| | `launch_executable` | Irreversible/consequential action | Starts an arbitrary executable with arguments; very high risk. |
| | `resize` | Reversible action | |
...
*   **Implementation**: Only one active "session" can hold the lock for input/consequential tools. State-producing read tools (like `Snapshot`) **must participate in the lock**. Why: The desktop state cache is a singleton used to resolve UI labels for input tools (`Click`, `Type`, `Move`, `Multi`). If Agent B calls `Snapshot` between Agent A's snapshot and Agent A's label-based action, B replaces the global cache. Agent A will then target the wrong element or receive an error, even though A holds the input lock. By making `Snapshot` participate in the lock, we prevent this race condition simply, without rewriting the core desktop state architecture to be session-scoped. Other completely stateless read tools (like `Process.list`) can remain unlocked. If a locked agent attempts to use an input tool, the server returns an explicit refusal. We will not queue inputs. Refusal is the safest and most predictable approach.
